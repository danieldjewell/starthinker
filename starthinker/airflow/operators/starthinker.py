  ...
  """
    ...
    def __init__(self,
        ...
        starthinker_callable = getattr(
            importlib.import_module('starthinker.task.%s.run' %
                                    starthinker_task), starthinker_task)

        super(StarThinkerTaskOperator,
              self).__init__(python_callable=starthinker_callable,
                             *args,
                             **kwargs)
        ...
        if gcp_conn_id:
            ...
            if service_connection_extra[
                ...
                try:
                    ...
                    self.log.error(
                        'Failed parsing the Keyfile JSON for gcp_conn_id=%s',
                        self.gcp_conn_id)
                    raise e
            if service_connection_extra[
                    'extra__google_cloud_platform__project']:
