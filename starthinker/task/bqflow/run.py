        ...
        auth, api_call, {
            'bigquery': {
                'dataset':
                    endpoint['dataset'],
                'table':
                    endpoint['table'].replace('BQFlow__', 'BQFlow__ERRORS__')
            }
        })
        ...
        if 'table' in endpoint:
            ...
            api_call = {
                ...
                'api':
                    api,
                'version':
                    Discovery_To_BigQuery.preferred_version(
                        api, project.task.get('key')),
                'function':
                    function,
            }

            kwargs_list = get_rows(project.task['auth'],
            ...
            for kwargs in kwargs_list:
                ...
                if project.verbose:
                    print('BQFLOW API CALL:', api_call)

                google_api_initilaize(api_call)
                google_api_execute(project.task['auth'], api_call, results,
                                   errors)
