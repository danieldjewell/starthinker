...
def dt_header(dt_file):
    ...
    sample_data = next(
        object_get_chunks(project.task['auth'], path, HEADER_SIZE))
...
def dt_timestamp(dt_filepath):
    ...
    if parts:
        ...
        if len(dt_time) == 10:
            ...
            dt_time = project.timezone.localize(
                datetime.strptime(dt_time, '%Y%m%d'))
...
def dt_move_large(dt_file, dt_partition, jobs):
    ...
    for data_gz in object_get_chunks(
            project.task['auth'], '%s:%s' % (project.task['bucket'], dt_file)):
        ...
        jobs.append(
            io_to_table(project.task['auth'], project.id,
                        ...
                        BytesIO(view[:end].encode()), 'CSV', schema, 0,
                        disposition, False))
...
def dt():
    ...
    for path in (project.task['paths'] or ['']):
        ...
        print(path)
        for dt_object in object_list(project.task['auth'],
            ...
            if ((project.task.get('days') is None and
                ...
                (dt_time >
                 project.now - timedelta(days=project.task.get('days', 60),
