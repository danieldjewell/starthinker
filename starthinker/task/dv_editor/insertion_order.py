    ...
    for row in rows:
        if row['Action'] == "DELETE":
            ...
            if row['Performance_Goal_Type'] != row['Performance_Goal_Type_Edit']:
                ...
                insertion_order["performanceGoal"]["performanceGoalType"] = row[
    ...
    for patch in patches:
        ...
        try:
            if patch["action"] == "DELETE":
                ...
                ).insertionOrders().delete(**patch["parameters"]).execute()
                patch["success"] = response
            elif patch["action"] == "PATCH":
                response = API_DV360(project.task["auth_dv"]).advertisers(
                ).insertionOrders().patch(**patch["parameters"]).execute()
                patch["success"] = response["insertionOrderId"]
            elif patch["action"] == "INSERT":
                response = API_DV360(project.task["auth_dv"]).advertisers(
                ).insertionOrders().create(**patch["parameters"]).execute()
                patch["success"] = response["insertionOrderId"]
