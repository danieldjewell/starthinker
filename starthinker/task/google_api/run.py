...
def google_api():
    ...
    results = google_api_build_results(project.task['auth'], api_call,
                                       project.task.get('results', {}))

    errors = google_api_build_errors(project.task['auth'], api_call,
                                     project.task.get('errors', {}))

    if 'kwargs' in project.task:
        kwargs_list = project.task['kwargs'] if isinstance(
            project.task['kwargs'],
            (list, tuple)) else [project.task['kwargs']]
