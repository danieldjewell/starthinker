    ...
    for row in (sheet or report).rows:
        ...
        for cell in row.cells:
            ...
            if columns[cell.column_id if sheet else cell.virtual_column_id][
                    'type'] == 'DATE' and value is not None:
                if 'T' in value:
...
def get_report_rows(token, report):
    ...
    while count < total:
        report_json = api.Reports.get_report(report,
                                             page_size=SMARTSHEET_PAGESIZE,
                                             page=page)
...
def smartsheet():
    ...
    if 'sheet' in project.task:
        ...
        rows = get_sheet_rows(project.task['token'], project.task['sheet'],
                              link)
        ...
        schema = get_report_schema(project.task['token'],
                                   project.task['report'])
    ...
    if 'bigquery' in project.task['out']:
        ...
        if link and 'schema' in project.task['out']['bigquery']:
            project.task['out']['bigquery']['schema'].insert(
                0, {
                    'name': 'rowPermalink',
                    'type': 'STRING',
                    'mode': 'NULLABLE'
