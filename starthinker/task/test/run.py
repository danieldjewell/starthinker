...
def schema_compare(expected, actual, path=''):
        ...
        for actual_column in actual:
            if expected_column['name'] == actual_column['name']:
                ...
                if expected_column.get('type') != actual_column.get('type'):
                    delta[make_path(expected_column['name'])] = {
                        'path': make_path(expected_column['name']),
                        'error': 'type',
                ...
                if expected_column.get('mode') != actual_column.get('mode'):
                    delta[make_path(expected_column['name'])] = {
                        'path': make_path(expected_column['name']),
                        'error': 'mode',
                        'expected': expected_column.get('mode'),
                        'actual': actual_column.get('mode')
                    }
                delta.update(
                    schema_compare(expected_column.get('fields', []),
                                   actual_column.get('fields', []),
                                   make_path(expected_column['name'])))
    ...
    for actual_column in actual:
        if actual_column['name'] not in matched:
            delta[make_path(actual_column['name'])] = {
                'path': make_path(actual_column['name']),
                'error': 'extra',
                'expected': '',
                'actual': actual_column['name']
            }
