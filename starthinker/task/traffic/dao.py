...
class BaseDAO(object):
  ...
  """
    ...
    """
        ...
        return self._api().get(profileId=self.profile_id,
                               id=feed_item[self._id_field]).execute()
            ...
            if items and len(items) > 0:
                ...
                if search_string == item['name']:
                    if len(items) > 1 and items[1]['name'] == search_string:
                        raise Exception(
                            'ERROR: More than one item found with %s %s' %
    ...
    """
        #print('ITEM', item)
        return self._api().insert(profileId=self.profile_id,
                                  body=item).execute()
