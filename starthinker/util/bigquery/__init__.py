    ...
    if not parameters:
        ...
        while '[PARAMETER]' in query:
            try:
                ...
                raise IndexError(
                    'BigQuery: Missing PARAMETER values for this query.')
    ...
    if job:
        ...
        while True:
            ...
            if 'errors' in result['status']:
                raise Exception('BigQuery Job Error: %s' % ' '.join(
...
def datasets_create(auth, project_id, dataset_id):
    ...
    API_BigQuery(auth).datasets().insert(projectId=project_id,
                                         body=body).execute()
...
def datasets_access(auth,
    ...
    if emails or groups or views:
        ...
        API_BigQuery(auth).datasets().patch(projectId=project_id,
                                            datasetId=dataset_id,
                                            body={
                                                'access': access
                                            }).execute()
...
def query_to_view(auth,
    ...
    response = API_BigQuery(auth).tables().insert(projectId=project_id,
                                                  datasetId=dataset_id,
                                                  body=body).execute()
    if response is None and replace:
        return API_BigQuery(auth).tables().update(projectId=project_id,
                                                  datasetId=dataset_id,
                                                  tableId=view_id,
                                                  body=body).execute()
...
def storage_to_table(auth,
    ...
    job = API_BigQuery(auth).jobs().insert(projectId=project_id,
                                           body=body).execute()
    if wait:
        try:
            job_wait(auth, job)
...
def rows_to_table(auth,
    ...
    writer = csv.writer(buffer_writer(buffer_data),
                        delimiter=',',
                        quotechar='"',
    ...
    for is_last, row in flag_last(rows):
        ...
        if is_last or buffer_data.tell() + 1 > BIGQUERY_BUFFERSIZE:
            ...
            io_to_table(auth, project_id, dataset_id, table_id, buffer_data,
                        'CSV', schema, skip_rows, disposition)
...
def table_create(auth,
    ...
    API_BigQuery(auth).tables().insert(projectId=project_id,
                                       datasetId=dataset_id,
                                       body=body).execute()


def table_get(auth, project_id, dataset_id, table_id):
    return API_BigQuery(auth).tables().get(projectId=project_id,
                                           datasetId=dataset_id,
...
def table_list(auth, project_id, dataset_id=None):
    if dataset_id is None:
        datasets = [
            ...
            for d in API_BigQuery(auth).datasets().list(
                projectId=project_id,
    ...
    for dataset_id in datasets:
        for table in API_BigQuery(auth, iterate=True).tables().list(
            ...
            yield table['tableReference']['datasetId'], table['tableReference'][
                'tableId'], table['type']
...
def table_delete(auth, project_id, dataset_id, table_id):
    try:
        API_BigQuery(auth).tables().delete(projectId=project_id,
                                           datasetId=dataset_id,
                                           tableId=table_id).execute()
        return True
    except HttpError as e:
...
def query_to_rows(auth,
    ...
    response = API_BigQuery(auth).jobs().query(projectId=project_id,
                                               body=body).execute()
