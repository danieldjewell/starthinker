...
class EmailTemplate:
    ...
    def __init__(self, template={}):
        ...
        self.copyright = template.get(
            'copyright', 'Copyright &copy; %d' % datetime.today().year)
    ...
    def _header_css(self):
        return (
            'width:100%%;margin:10px 0px 20px '
            '0px;padding:0px;border:0px;text-align:%(align)s;color:%(text)s;font-family:%(font)s;font-size:22px;line-height:30px;font-weight:500;letter-spacing:-0.31px;border:0px;'
        ) % self.style

    def _text_css(self):
        return (
            'width:100%%;margin:10px '
            '0px;padding:0px;border:0px;font-weight:normal;text-align:%(align)s;color:%(text)s;font-family:%(font)s;font-size:15px;line-height:26px;'
        ) % self.style
    ...
    def _button_css(self):
        return (
            ...
            ' '
            '19px;background-color:%(link)s;color:%(background)s;font-family:%(font)s;font-size:16px;font-weight:bold;letter-spacing:-0.31px;text-decoration:none;text-align:center;'
        ) % self.style
    ...
    def markup_to_text(self, markup):
        return MARKUP_TO_TEXT.sub(r'\1 (\2)',
                                  markup).replace('\n', '<br/><br/>')
    ...
    def header(self, text):
        ...
        self.content_html += '<p style="%s">%s</p>' % (
            self._header_css(), self.markup_to_html(text))
    ...
    def column(self, number=0):
        if number:
            ...
            self.content_html += (
                '<td '
                'style="width:%d%%;padding:10px;text-align:center;vertical-align:top;">'
            ) % width
        else:
    ...
    def list(self, elements):
        ...
        for element in elements:
            self.content_html += (
                ...
                'solid #ccc;%s">%s</td></tr>') % (self.style['align'], border,
                                                  self._table_css(),
    ...
    def table(self, schema, rows):
        ...
        for header in schema:
            self.content_html += (
                ...
                ' solid #ccc;font-weight:bold;%s">%s</td>') % (aligns.get(
                    header['type'], 'left'), self._table_css(), header['name'])
        self.content_html += '</tr>'
        ...
        for row in rows:
            ...
            for column, header in enumerate(schema):
                ...
                if isinstance(value, str) and value.startswith('http'):
                    value = (
                        '<a href="%s" style="border-radius:50px;padding:1px '
                        '12px;%s">&rarr;</a>') % (value, self._button_css())
                self.content_html += (
                    '<td '
                    'style="padding:10px;text-align:%s;border-bottom:1px'
                    ' solid #ccc;%s">%s</td>') % (aligns.get(
                        header['type'], 'left'), self._table_css(), value)
            self.content_html += '</tr>'
    ...
    def get_html(self):
        ...
        if self.footer:
            ...
            for f in self.footer:
                footer += (
                    '<a href="%s" style="display:inline-block;margin: '
                    '0px;padding:0px '
                    '10px;color:%s;font-family:%s;font-size:15px;font-weight:normal;line-height:32px;text-decoration:none;">%s</a>'
                ) % (f['link'], self.style['text'], self.style['font'],
                     f['text'])
            footer += '</p>'
        if self.copyright:
            footer += (
                '<p style="width:100%%;margin:20px '
                'auto;padding:0px;border:0px;font-weight:bold;text-align:center;color:%(text)s;font-family:%(font)s;font-size:15px;line-height:32px;">'
            ) % self.style
