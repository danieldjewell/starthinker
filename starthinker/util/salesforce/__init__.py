...
def authenticate(domain, client_id, client_secret, username, password):
    ...
    req = request.Request('https://%s/services/oauth2/token' % domain,
                          data=data)
    creds = json.loads(request.urlopen(req).read())
    return Salesforce(instance_url=creds['instance_url'],
...
def query(service, query, header=False):
    for record in service.query_all(query)['records']:
        if header:
            ...
            header = False
        yield [
            record[column] for column in record.keys() if column != 'attributes'
        ]
