...
class SetupForm(forms.ModelForm):
    week = ListChoiceField(choices=map(lambda d: (d[:3], d), DAYS),
                           initial=map(lambda d: d[:3], DAYS))
    hour = ListChoiceIntegerField(choices=map(lambda h: (h, h), HOURS),
                                  initial=[3])
    ...
    def __init__(self, manual, account, *args, **kwargs):
        ...
        if account.get_domain():
            query |= (Q(share='domain') & ~Q(account__domain='') &
                      Q(account__domain=account.get_domain()))
        self.fields['project'].queryset = Project.objects.filter(
            query).order_by('share', 'identifier')

        self.fields[
            'name'].help_text = 'Identify this recipe in list of recipes.'
        self.fields['project'].help_text = (
            'Choose a <b>Google Cloud Project Service '
