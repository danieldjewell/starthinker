...
def worker_status(worker_uid, recipe_uid, script, instance, hour, event, stdout,
    ...
    try:
        ...
        print('Expired Worker Job:', worker_uid, recipe_uid, script, instance,
              hour, event)
...
class Workers():
    ...
    def ping(self):
        ...
        while MANAGER_HEALTHY and not self.ping_event.wait(
        ...
        for job in self.jobs:
            ...
            if status:
                worker_status(job['job']['worker'],
                              job['recipe']['setup']['uuid'], job['script'],
                              job['instance'], job['hour'], status, stdout,
                              stderr)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            help=
            'Maximum number of jobs simlutanelous processes to start within this worker.',
        )
        ...
        parser.add_argument(
            ...
            type=int,
            help=
            'Default seconds to allow a task to run before timing it out, also controlled by recipe.',
        )
