...
class Recipe(models.Model):
    ...
    def get_days(self):
        return json.loads(self.week or '[]') or [
    ...
    def get_json(self, credentials=True):
        return Script.get_json(
            ...
            self.timezone, self.get_days(), self.get_hours(), self.get_values())
    ...
    def get_job_utm(self, status):
        ...
        now_tz = now_tz.replace(hour=hour or 0,
                                minute=0,
                                second=0,
                                microsecond=0)
    ...
    def get_status(self, update=False, force=False, cancel=False):
        ...
        # if manual and all task are done set the utm to be ignored in worker pulls
            ...
            if not status['tasks'] or all(
                    task['done'] for task in status['tasks']):
                self.job_utm = 0
        ...
        # if updating, modify the status
            ...
            for order, (script, task) in enumerate(self.get_tasks()):
                ...
                hours = [now_tz.hour] if force else task.get(
                    'hour', self.get_hours())
    ...
    def set_task(self, script, instance, hour, event, stdout, stderr):
        ...
        for task in status['tasks']:
            if task['script'] == script and task[
                    'instance'] == instance and task['hour'] == hour:
        ...
        if self._cache_log is None:
            ...
            for task in self._cache_log['tasks']:
                ...
                if task['event'] == 'JOB_TIMEOUT':
                    ...
                    new = True
                elif task['event'] not in ('JOB_PENDING', 'JOB_START',
                                           'JOB_END'):
                    error = True
