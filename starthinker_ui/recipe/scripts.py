...
class Script:
    ...
    def get_link_ui(self):
        return '/recipe/%s/?script=%s' % ('manual' if self.is_manual() else
                                          'edit', self.get_tag())
    ...
    def get_released(self):
        try:
            return datetime.strptime(
                self.script.get('script', {}).get('released', ''),
                '%Y-%m-%d').date()
        except:
            return None
    ...
    @staticmethod
    def get_json(uuid, project_id, project_key, credentials_user,
                 credentials_service, timezone, days, hours, values):
        tasks = []
        hours = set(hours)
        for v in values:
...
if __name__ == '__main__':
  ...
  """
    ...
    content = ('# DO NOT EDIT, GENERATED BY: python '
              ...
              ) % pprint.pformat(SCRIPTS, indent=1)
